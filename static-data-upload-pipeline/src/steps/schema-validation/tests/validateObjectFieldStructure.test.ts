...
import { validateObjectFieldStructure } from '../utils';

describe('validateObjectFieldStructure', () => {
  ...
  describe('field type validation', () => {
    it('should fail with invalid field type containing special characters', () => {
      ...
      const errors = validateObjectFieldStructure(field, 'testField', 'stats', 'testGroup', mockGroup, mockSchema);
      expect(errors).toHaveLength(2);
      expect(errors[0].message).toContain('Object field type must contain only letters and digits');
      expect(errors[1].message).toContain('Object field type must be one of: String, Boolean, Ref, Object');
    });
    ...
    it('should fail with disallowed field type', () => {
      ...
      expect(errors[0].message).toContain('Object field type must be one of: String, Boolean, Ref, Object');
    });
  ...
  describe('Ref field name conflicts', () => {
    it('should fail when Ref field conflicts with existing field', () => {
      ...
      const errors = validateObjectFieldStructure(field, 'weaponsRef', 'stats', 'testGroup', mockGroup, mockSchema);
      expect(errors).toHaveLength(1);
    ...
    it('should pass when Ref field does not conflict', () => {
      ...
      const errors = validateObjectFieldStructure(field, 'armourRef', 'stats', 'testGroup', mockGroup, mockSchema);
      expect(errors).toHaveLength(0);
    });
  });
  ...
  describe('combined validation', () => {
    it('should return multiple errors for invalid field', () => {
      ...
      const errors = validateObjectFieldStructure(field, 'weaponsRef', 'stats', 'testGroup', mockGroup, mockSchema);
      expect(errors).toHaveLength(4); // invalid type, invalid objName, invalid refTo, Ref conflict
    });
  });
});
